...
class TestClient(object):
    ...
    def test_client_fetch_file(self, wsman_conn):
        ...
        try:
            ...
            expected_hash = b"\x70\xe3\xbe\xa8\xcd\xb0\xd0\xc8" b"\x83\xbc\xcf\xf5\x22\x89\x33\xd9" b"\x33\xb8\x8a\x80"
            hash = hashlib.sha1()
    ...
    def test_client_fetch_file_fail_missing(self, wsman_conn):
        ...
        assert (
            str(err.value) == "Failed to fetch file C:\\fakefile.txt: The path at " "'C:\\fakefile.txt' does not exist"
        )
    ...
    def test_client_fetch_file_hash_mismatch(self, wsman_conn, monkeypatch):
        # set to a hash that is not the actual to verify the script will
        # fail in a hash mismatch scenario
        mock_hash = MagicMock()
        mock_hash.return_value.hexdigest.return_value = "c3499c2729730a7f807efb8676a92dcb6f8a3f8f"
    ...
    def test_client_fetch_file_expand_vars(self, wsman_conn):
        ...
        try:
            ...
            expected_hash = b"\x22\xC1\xA7\x8E\xC5\xAD\xA1\xCD" b"\x2F\x36\x65\xB5\x8B\x30\x49\x9E" b"\x51\xA3\xB0\x29"
