...
process {
    ...
    def test_psrp_error_failed(self, wsman_conn):
        with RunspacePool(wsman_conn) as pool:
            ps = PowerShell(pool)
            ps.add_script(
                "$ErrorActionPreference = 'Stop'; " "Write-Output before; " "Write-Error error; Write-Output after"
            )
            actual = ps.invoke()
...
class TestFragmenter(object):
    ...
    def test_fragment_one_multiple_fragments(self):
        ...
        assert actual == [
            ...
            b"\x00\x00\x00\x00\x00\x00\x00\x00"
            b"<S>1234</",
            b"\x00\x00\x00\x00\x00\x00\x00\x01" b"\x00\x00\x00\x00\x00\x00\x00\x01" b"\x02" b"\x00\x00\x00\x02" b"S>",
        ]
    ...
    def test_fragment_multiple(self):
        ...
        assert actual == [
            ...
            b"\x00\x00\x00\x00\x00\x00\x00\x04" b"\x00\x00\x00\x00\x00\x00\x00\x01" b"\x02" b"\x00\x00\x00\x02" b"S>",
        ]
